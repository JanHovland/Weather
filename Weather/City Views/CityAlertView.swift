...
struct CityAlertView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationBarTitle(Text(NSLocalizedString("Alert View", comment: "CityAlertView")), displayMode: .large)
            .navigationBarItems(leading:
                                    Button(action: {
                                        /// Rutine for å returnere til personoversikten
                                        presentationMode.wrappedValue.dismiss()
                                    }, label: {
                                        ReturnFromMenuView(text: NSLocalizedString("Weather", comment: "CityAlertView"))
                                    }))
            .padding()
        }
    }
